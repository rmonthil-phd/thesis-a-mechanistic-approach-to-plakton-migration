...
The flow velocity $\FlowVelocity$ and pressure $p$ of such a flow are described by the the incompressible Navier-Stokes equations,
...
For real planktonic organisms, we may not expect them experiencing preferential alignment of vortices with the vertical.
Figure \ref{fig:taylor_green_vortex_tau_performance_vs_and_rdir}\textbf{(b)} shows performance of surfers averaged over both starting positions and flow orientation (or seamlessly over various target direction $\Direction$).
This averaged performance, noted $\left\langle \Performance \right\rangle_{N,\Direction}$, slightly decreases.
...
Note that the efficiency $\Performance / \SwimmingVelocity$ remains larger than 2 for the optimal time horizon $\TimeHorizon$.
\afterpage{
\begin{figure}
	\centering

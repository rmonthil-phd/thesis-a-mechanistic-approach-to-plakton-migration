\begin{tikzpicture}
	...
	\begin{axis} [
		...
		on layer=axis background,
		domain=0:3.5,
		]{(2 * exp(x * \modtimeasym * (0.5 * \modomega)^2 / (1.0 + (x * \modomega * 0.5)^2)) * cos(deg(0.5 * \modomega * (\modtimeasym - x))) + 1) / ((1.0 + (x * \modomega * 0.5)^2) * (2 * cos(deg(0.5 * \modomega * \modtimeasym)) + 1))};
		\addlegendentry{Eq.~(A.28)}
		%% y = x
		\addplot
		[
		color=gray!50!white,
